...
let rec fold_expr (f : 'a expr_f -> 'a) : expr -> 'a = function
  | ELit (ty, sp, lit) -> f (ELitF (ty, sp, lit))
  | EVar (ty, sp, name) -> f (EVarF (ty, sp, name))
  | ELet (ty, sp, name, def, body) ->
      let def = fold_expr f def in
      let body = fold_expr f body in
      f (ELetF (ty, sp, name, def, body))
  | ELetRec (ty, sp, name, def, body) ->
      let def = fold_expr f def in
      let body = fold_expr f body in
      f (ELetRecF (ty, sp, name, def, body))
  | EIf (ty, sp, cond, con, alt) ->
      let cond = fold_expr f cond in
      let con = fold_expr f con in
      let alt = fold_expr f alt in
      f (EIfF (ty, sp, cond, con, alt))
  | ELam (in_t, out_t, sp, param, body) ->
      let body = fold_expr f body in
      f (ELamF (in_t, out_t, sp, param, body))
  | EApp (ty, sp, func, arg) ->
      let func = fold_expr f func in
      let arg = fold_expr f arg in
      f (EAppF (ty, sp, func, arg))
  | EMatch (ty, sp, expr, alts) ->
      let expr = fold_expr f expr in
      let alts = List.map (fun (p, e) -> (p, fold_expr f e)) alts in
      f (EMatchF (ty, sp, expr, alts))

(*  *)
let lit_to_tast : lit -> Tast.lit = function
  | Int x -> Tast.Int x
  | Bool x -> Tast.Bool x
...
let expr_to_tast : expr -> Tast.expr =
  fold_expr (function
    | ELitF (ty, _, lit) -> Tast.ELit (ty, lit_to_tast lit)
    | EVarF (ty, _, var) -> Tast.EVar (ty, var)
    | ELetF (ty, _, name, def, body) -> Tast.ELet (ty, name, def, body)
    | ELetRecF (ty, _, name, def, body) -> Tast.ELetRec (ty, name, def, body)
    | EIfF (ty, _, cond, con, alt) -> Tast.EIf (ty, cond, con, alt)
    | ELamF (in_t, out_t, _, param, body) -> Tast.ELam (in_t, out_t, param, body)
    | EAppF (ty, _, func, arg) -> Tast.EApp (ty, func, arg)
    | EMatchF (ty, _, expr, cases) ->
        let cases = List.map (fun (p, e) -> (pat_to_tast p, e)) cases in
        Tast.EMatch (ty, expr, cases))

let ty_def_to_tast : ty_def -> Tast.ty_def = function
  | TyDef { name; kind; alts; _ } -> Tast.TyDef { name; kind; alts }

let tm_def_to_tast : tm_def -> Tast.tm_def = function
  | TmDef { name; scheme; expr; _ } ->
      let expr = expr_to_tast expr in
      Tast.TmDef { name; scheme; expr }

let modu_to_tast : modu -> Tast.modu = function
  | Module { name; types; terms; _ } ->
      let types = List.map ty_def_to_tast types in
      let terms = List.map tm_def_to_tast terms in
